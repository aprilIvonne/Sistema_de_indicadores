...
namespace prueba
{
    public partial class FormRRHH : Form
    {
        ...
        {
            TablaVerDatos buscar = new TablaVerDatos("rrhh", arregloCodigos);
            buscar.ShowDialog();
            if (buscar.seleccion != null)
            {
                ...
                foreach (Control txt in this.panel1.Controls)
                {
                    ...
                    if (txt is TextBox)
                    {
                        ...
                        /*if (!txt.Name.StartsWith("result"))
                        {
                            if (buscar.seleccion.ToArray().GetValue(cont).ToString() == "")
                            {
                                txt.Text = "";
                                txt.Enabled = false;
                            }
        ...
        public void Agregar(List<Control> ControlList, string area)
        {
            if (codigo == null)
            {
                MySqlConnection conectar = BDComun.ObtenerConexion();
        ...
        public void Update(List<Control> ControlList, string area, string cod)
        {
